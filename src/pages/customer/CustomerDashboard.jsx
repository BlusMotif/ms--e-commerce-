      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
        <div className="card bg-blue-50 border-l-4 border-blue-500">
          <div className="flex items-center justify-between gap-3">
            ...
            <ShoppingBag className="w-8 h-8 sm:w-10 sm:h-10 text-blue-500 flex-shrink-0" />
          </div>
        </div>
        ...
        <div className="card bg-yellow-50 border-l-4 border-yellow-500">
          <div className="flex items-center justify-between gap-3">
            <div className="flex-1 min-w-0">
              <p className="text-yellow-700 text-xs sm:text-sm font-medium mb-1">Pending</p>
              <p className="text-2xl sm:text-3xl font-bold text-yellow-900">{stats.pendingOrders}</p>
            </div>
            <Clock className="w-8 h-8 sm:w-10 sm:h-10 text-yellow-500 flex-shrink-0" />
          </div>
        </div>
        ...
        <div className="card bg-green-50 border-l-4 border-green-500">
          <div className="flex items-center justify-between gap-3">
            <div className="flex-1 min-w-0">
              <p className="text-green-700 text-xs sm:text-sm font-medium mb-1">Completed</p>
              <p className="text-2xl sm:text-3xl font-bold text-green-900">{stats.completedOrders}</p>
            </div>
        ...
        <div className="card bg-purple-50 border-l-4 border-purple-500">
          <div className="flex items-center justify-between gap-3">
            <div className="flex-1 min-w-0">
              <p className="text-purple-700 text-xs sm:text-sm font-medium mb-1">Total Spent</p>
              <p className="text-xl sm:text-2xl font-bold text-purple-900">GH₵ {stats.totalSpent.toFixed(2)}</p>
            </div>
            <DollarSign className="w-8 h-8 sm:w-10 sm:h-10 text-purple-500 flex-shrink-0" />
          </div>
        </div>
      </div>

      {/* Orders List with better spacing */}
      <div className="card">
        ...
        {orders.length > 0 ? (
          <div className="space-y-3 sm:space-y-4">
            {orders.map((order) => (
              <div key={order.id} className="border border-gray-200 rounded-lg p-3 sm:p-4 hover:shadow-md transition">
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 mb-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-2">
                      <h3 className="font-bold text-gray-900 text-sm sm:text-base">Order #{order.id.substring(0, 8)}</h3>
                  ...
                  <div className="flex items-center justify-between sm:flex-col sm:items-end gap-3">
                    <p className="text-xl sm:text-2xl font-bold text-green-600">GH₵ {order.total.toFixed(2)}</p>
                    <button
                      onClick={() => handleViewOrder(order)}
                      className="px-3 sm:px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition flex items-center gap-2 text-xs sm:text-sm whitespace-nowrap"
                    >
                ...
                {/* Order Items Preview - More Compact */}
                <div className="border-t pt-2 sm:pt-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                    ...
                    {order.items?.length > 3 && (
                      <div className="flex items-center justify-center bg-gray-100 p-2 rounded text-xs sm:text-sm text-gray-600">
          ...
          <div className="text-center py-8 sm:py-12">
            ...
            <p className="text-gray-500 text-xs sm:text-sm mt-2">Start shopping to see your orders here!</p>
          </div>
        )}
      </div>
